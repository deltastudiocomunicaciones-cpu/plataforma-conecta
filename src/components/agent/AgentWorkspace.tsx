...
import { FileText, Search, CalendarDays, ChartNoAxesCombined, ArrowUpRight, Sparkles } from "lucide-react";
...
export function AgentCapabilityCard({ capability, onSelect }: { capability: AgentCapability; onSelect: (capability: AgentCapability) => void }) {
  ...
  return <button type="button" className="agent-capability" onClick={() => onSelect(capability)}>
    <span className="agent-capability-icon"><Icon size={22} aria-hidden="true" /></span><ArrowUpRight className="agent-card-arrow" size={17} aria-hidden="true" /><strong>{capability.title}</strong><span>{capability.description}</span><small>Explorar capacidad</small>
  </button>;
}
...
export function AgentWorkspace({ context, suggestions = [] }: { context: AgentContext; suggestions?: readonly AgentSuggestion[] }) {
  ...
  return <>
    ...
    <header className="agent-panel agent-intro"><div className="agent-intro-symbol" aria-hidden="true"><Sparkles size={40} /></div><p className="eyebrow">CONECTA INTELIGENCIA · TU ESPACIO DE TRABAJO</p><h1>Hola, {name}.<br /><span>Dale espacio a tus ideas.</span></h1><p>El contexto de tu cargo, tus herramientas y tu próxima conversación, en un mismo lugar.</p><span className="agent-status">Agente por configurar</span><p>La asistencia estará disponible cuando se conecten las capacidades y el conocimiento autorizado de tu cargo.</p><ul className="agent-principles"><li>01 · Explora tu cargo</li><li>02 · Descubre las capacidades</li><li>03 · Prepara tu próxima consulta</li></ul></header>
    <div className="agent-columns"><div className="agent-main"><AgentConversation name={name} />
    ...
    </div><aside className="agent-context-rail"><AgentSuggestions suggestions={suggestions} />
